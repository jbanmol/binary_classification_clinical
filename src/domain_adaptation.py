...
class DomainShiftDetector:
    ...
    def comprehensive_shift_analysis(self, source_data: pd.DataFrame,
        ...
        numeric = list(set(source_data.select_dtypes(include=[np.number]).columns)
                       & set(target_data.select_dtypes(include=[np.number]).columns))
        cov = {'covariance_difference_norm': 0.0, 'relative_covariance_shift': 0.0}
        if len(numeric) > 1:
            cov = self.detect_covariance_shift(source_data[numeric].fillna(0).values,
                                               target_data[numeric].fillna(0).values)

        # simple heuristic for overall severity
        ...
        moderate = (ks_over + w_over + m_over) == 1 or any(
            v['ks_statistic'] > 0.2 or v['wasserstein_distance'] > 0.5 or v['mean_shift_std'] > 1.5
...
class DomainAdapter:
    ...
    def coral_alignment(self, source_features: np.ndarray,
        ...
        # CORAL: whiten source, color with target
